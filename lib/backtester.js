...
function calculateStats(trades, finalEquity, maxDrawdown, config, costs, totalCosts) {
  ...
  const kelly = calcKellyCriterion(winRateDecimal, avgWin, avgLoss)
  const optimalLeverage = calcOptimalLeverage(winRateDecimal, avgWin, avgLoss, 20)
...
export function formatResults(result) {
  ...
  if (result.kelly && result.kelly.kellyPct > 0) {
    lines.push('')
    lines.push('───────────────────────────────────────────────────────────')
    lines.push('                   KELLY CRITERION')
    lines.push('───────────────────────────────────────────────────────────')
    lines.push('')
    lines.push(`Win/Loss Ratio:     ${result.kelly.winLossRatio?.toFixed(2) || 'N/A'}`)
    lines.push(`Edge per Trade:     ${result.kelly.edge >= 0 ? '+' : ''}${result.kelly.edge?.toFixed(2) || 0}%`)
    lines.push(`Full Kelly:         ${result.kelly.kellyPct?.toFixed(1) || 0}% of capital`)
    lines.push(`Half Kelly:         ${result.kelly.halfKelly?.toFixed(1) || 0}% (recommended)`)
    lines.push(`Quarter Kelly:      ${result.kelly.quarterKelly?.toFixed(1) || 0}% (conservative)`)
    if (result.optimalLeverage?.recommendation) {
      lines.push('')
      lines.push(`Suggestion:         ${result.optimalLeverage.recommendation}`)
    }
  } else if (result.kelly) {
    lines.push('')
    lines.push('───────────────────────────────────────────────────────────')
    lines.push('                   KELLY CRITERION')
    lines.push('───────────────────────────────────────────────────────────')
    lines.push('')
    lines.push('⚠️  No positive edge detected - Kelly suggests 0% sizing')
  }
