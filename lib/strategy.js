 ...
 *
 * @param {number} winRate - Win rate as decimal (0.6 = 60%)
 * @param {number} avgWin - Average winning trade return %
 * @param {number} avgLoss - Average losing trade return % (negative)
 * @returns {{ kellyPct: number, halfKelly: number, quarterKelly: number, edge: number }}
 */
export function calcKellyCriterion(winRate, avgWin, avgLoss) {
  // Handle edge cases
  if (winRate <= 0) {
    return { kellyPct: 0, halfKelly: 0, quarterKelly: 0, edge: 0, winLossRatio: 0 }
  }

  // 100% win rate - can't calculate normal Kelly (no losses)
  // Use simplified approach: risk proportional to edge
  if (winRate >= 1 || avgLoss >= 0) {
    const edge = avgWin
    // Cap at 50% for safety even with "perfect" results
    const kellyPct = Math.min(50, edge)
    return {
      kellyPct,
      halfKelly: kellyPct / 2,
      quarterKelly: kellyPct / 4,
  ...
  const W = winRate
  const R = avgWin / Math.abs(avgLoss) // Win/Loss ratio
  ...
  return {
    kellyPct: Math.max(0, kellyPct * 100), // As percentage, floor at 0
    halfKelly: Math.max(0, (kellyPct / 2) * 100),
    quarterKelly: Math.max(0, (kellyPct / 4) * 100),
    winLossRatio: R,
 ...
 *   Optimal Leverage = Kelly% × (Expected Return / Variance)
 *
 * More practically: Kelly tells you what % of capital to risk,
 * and leverage amplifies both gains and losses.
 *
 * @param {number} winRate - Win rate as decimal
 * @param {number} avgWin - Average win % (unleveraged price move)
 * @param {number} avgLoss - Average loss % (unleveraged price move, negative)
 * @param {number} maxLeverage - Maximum allowed leverage
 * @returns {{ optimalLeverage: number, kellyFraction: number, recommendation: string }}
 */
export function calcOptimalLeverage(winRate, avgWin, avgLoss, maxLeverage = 20) {
  ...
  // For leveraged perps, Kelly fraction represents capital at risk
  // If Kelly says 20% of capital, and we use 10x leverage, we're risking 2% of capital per 1% move
  // Optimal sizing: Kelly% / (leverage × avg_loss%)

  // Simple approach: Half-Kelly is safer for real trading
  const halfKellyFraction = kelly.halfKelly / 100

  // Estimate variance from win/loss magnitudes
  const variance = (winRate * Math.pow(avgWin, 2)) + ((1 - winRate) * Math.pow(avgLoss, 2))
  const stdDev = Math.sqrt(variance)

  // Optimal leverage scales inversely with volatility
  // Kelly-optimal leverage ≈ edge / variance
  const rawOptimalLeverage = kelly.edge / variance * 100

  // Cap at max leverage and use half-Kelly for safety
  const optimalLeverage = Math.min(Math.max(1, rawOptimalLeverage / 2), maxLeverage)

  let recommendation
  if (kelly.kellyPct > 50) {
    recommendation = `Strong edge. Half-Kelly suggests ${kelly.halfKelly.toFixed(1)}% position size`
  } else if (kelly.kellyPct > 25) {
    recommendation = `Good edge. Consider ${kelly.halfKelly.toFixed(1)}% position size with ${optimalLeverage.toFixed(1)}x leverage`
  } else if (kelly.kellyPct > 10) {
    recommendation = `Moderate edge. Use quarter-Kelly (${kelly.quarterKelly.toFixed(1)}%) for safety`
  } else if (kelly.kellyPct > 0) {
    recommendation = `Thin edge. Small position sizes recommended (${kelly.quarterKelly.toFixed(1)}%)`
  } else {
    recommendation = 'No edge detected'
  }

  return {
    optimalLeverage: Math.round(optimalLeverage * 10) / 10,
    kellyFraction: kelly.kellyPct,
